...
1. Search for `'farquest'` (inside single quotations) to capture the text domain.
2. Search for `farquest_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `farquest-` to capture prefixed handles.

OR

* Search for: `'farquest'` and replace with: `'megatherium'`
* Search for: `farquest_` and replace with: `megatherium_`
